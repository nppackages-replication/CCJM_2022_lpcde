...
simtest = function(s){
  ...
  bw_star = lpcde::lpbwcde(y_data=y_data, x_data=x_data, y_grid=y_grid, x=x,
                           p=3, mu=0, bw_type="imse-rot")$BW[,2]

  # true parameters
  true_val = stats::pnorm(y_grid, mean = -0.1*x, sd=sqrt(1-0.1^2))
  ...
  # standard deviation
  sd = (abs(est$Estimate[,5]))

  # average length of confidence interval
  aw_pw = 2*1.96*sd

  # confidence band computation
  corrMat = sweep(sweep(est$CovMat$CovMat, MARGIN=1, FUN="*", STATS=1/est$Estimate[, "se"]),
  ...
  # standard deviation
  sd_rbc = 2*(abs(est$Estimate[,6]))

  # CI length
  aw_pw_rbc = 2*1.96*sd_rbc

  # CB computation
  corrMat = sweep(sweep(est$CovMat$CovMat_RBC, MARGIN=1, FUN="*", STATS=1/est$Estimate[, "se_RBC"]),
  ...
  results = matrix(c(bw_star, bias, sd, ci_size, cb_size, aw_pw, aw_u,
                     bw_star, bias_rbc, sd_rbc, ci_size_rbc, cb_size_rbc, aw_pw_rbc, aw_u_rbc),
                   ncol = 14)
  return(results)
}
...
for (x in c(0, 0.8, 1.0)){
  cat(x)
  # running simulations
  object = mclapply(1:num_sims, simtest, mc.cores = 1)
...
for (x in c(0, 0.8, 1.0)){
  ...
  # running simulations
  object = parallel::mclapply(1:num_sims, simtest, mc.cores = 1)
...
simtest = function(s){
  ...
  # true parameters
  true_val = -(y_grid+0.1*x)*stats::dnorm(y_grid, mean = -0.1*x, sd=sqrt(1-0.1^2))/(1-0.1^2)
  ...
  # CB computation
  corrMat = sweep(sweep(est$CovMat$CovMat_RBC, MARGIN=1,
                        FUN="*", STATS=1/est$Estimate[, "se_RBC"]),
                  MARGIN=2, FUN="*", STATS=1/est$Estimate[, "se_RBC"])
  normalSimu = try(
    MASS::mvrnorm(n=CIsimul, mu=rep(0,nrow(corrMat)),
                  Sigma=Matrix::nearPD(corrMat)$mat),
...
for (x in c(0, 0.8, 1.0)){
  cat(x)
  # running simulations
  object = mclapply(1:num_sims, simtest, mc.cores = numCores)
