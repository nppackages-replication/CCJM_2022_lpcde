...
h_grid = c(0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3)

bw_testing = function(s){
  # simulating data
  data = mvtnorm::rmvnorm(5*n, sigma = matrix(c(2, -0.1, -0.1, 2), nrow=2))
  data = data[abs(data[,1])<=1.2,]
  data = data[abs(data[,2])<=1.2,]
  data = data[1:n, ]
...
for (i in 1:length(h_grid)){
 h = h_grid[i]
 object = mclapply(1:num_sims, bw_testing, mc.cores =1)
